...
class DistributionTab(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        row(fl, "Répartition", self.cb_pr, TOOLTIPS["distribution.pr"], lambda: self.cb_pr.setCurrentText(d["pr"]))
        row(fl, "Distance minimale (px)", self.sp_dmin_px, TOOLTIPS["distribution.dmin_px"], lambda: self.sp_dmin_px.setValue(d["dmin_px"]))
